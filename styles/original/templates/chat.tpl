...
<link rel="stylesheet" type="text/css" href="${tplroot}/chat.css?20131217">
...
//-->
    ...
    function score(quality, obj, threadid) {

        if (typeof(my_ajax) != "undefined") {
            my_ajax.abort();
            my_ajax2.abort();
        }

        my_ajax = jQuery.ajax({
            url: 'getAgentEmail.php',
            type: 'POST',
            data: {threadid: threadid},
            cashe: false,
            success: function(response_wi) {

                if (typeof(my_ajax2) != "undefined") {
                    my_ajax2.abort();
                }

                var id_score=jQuery("#id_score").val();
                var key=jQuery("#key").val();
                var comment_user=jQuery("#id_comment").val();
                jQuery(".score_divbox").css({'background':'#f9f9f9'});

                my_ajax2 = jQuery.ajax({
                    url: 'http://feedback.from.sh/action/score_action_chat.php',
                    type: 'POST',
                    crossDomain: true,
                    data: {quality: quality, id_score: id_score, key: key, comment_user: comment_user, worker_id: response_wi},
                    dataType: 'json',
                    cache: false,
                    success: function(response){
                        if (response.message==true) {
                            obj.style.background='#ffca10';
                        }
                    }
                });
            }
...
${if:canpost}
            <div style="float: left; height: 100px;">
                ...
                <input type="hidden" id="id_score" value="${page:ct.chatThreadId}"></input>

                <div id="score_buttons">
                    <p style="font-size: 12px;">Просим Вас высказать свое мнение о консультации</p>

                    <div class="score_divbox" style="margin-left: 10px;" onClick="score('+1', this, '${page:ct.chatThreadId}');"  onMouseDown="this.style.padding='7px 3px 3px 7px';" onMouseUp="this.style.padding='5px';">
                        <p>Вы большие молодцы!</p>
                    </div>

                    <div class="score_divbox" onClick="score('0', this, '${page:ct.chatThreadId}');" onMouseDown="this.style.padding='7px 3px 3px 7px';" onMouseUp="this.style.padding='5px';">
                        <p>Нормально!</p>
                    </div>

                    <div class="score_divbox" onClick="score('-1', this, '${page:ct.chatThreadId}');"  onMouseDown="this.style.padding='7px 3px 3px 7px';" onMouseUp="this.style.padding='5px';">
                        <p>Вам есть что улучшать!</p>
                    </div>
