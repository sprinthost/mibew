...
<?php
    ...
    if(!defined("L_LANG") || L_LANG == "L_LANG")
    {
        if($hl) define("L_LANG", $hl);
            ...
            else define("L_LANG", "ru_RU"); // Ebraic example - change the red value to your desired language (from the list provided)
    }
	...
	<div class="fieldForm">
		<div class="field">
			...
			<div class="fvaluenodesc">
                ...
                <div id="calendar1" name="calendar1" style="padding-top:5px;">
					...
					<?php
                        ...
                        $myCalendar->setDate(date("d", strtotime($date1_d))
                            , date("m", strtotime($date1_d))
                            , date("Y", strtotime($date1_d)));
                        $myCalendar->setPath("calendar/");
                        $myCalendar->setYearInterval(2007, 2020);
                ...
                <div id="calendar2">
                    <div style="float: left;"><pre> -> </pre></div>
					<div style="float: left;">
					<?php
                         $date2_d = $_SESSION['d2'];
                         $myCalendar = new tc_calendar("date2", true, false);
                         $myCalendar->setIcon("calendar/images/iconCalendar.gif");
                         $myCalendar->setDate(date("d", strtotime($date2_d))
                            , date("m", strtotime($date2_d))
							, date("Y", strtotime($date2_d)));
                         $myCalendar->setPath("calendar/");
                         $myCalendar->setYearInterval(2007, 2020);
                         $myCalendar->setAlignment("left", "bottom");
                         //$myCalendar->setDatePair("date1", "date2", $date2_default);
